...
function updateOfflineIndicator() {
  ...
  if (!navigator.onLine) {
    ...
    // (вырез/динамический остров) — меряем её и отдаём в CSS-переменную,
    // чтобы контент отодвинулся ровно настолько и капсула не перекрывала
    // верхнюю плашку с кнопками
    const h = indicator.offsetHeight;
    document.documentElement.style.setProperty('--offline-indicator-height', h + 'px');
