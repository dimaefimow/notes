...
const TIMER_ENABLED_KEY = 'tasksApp_timerEnabled_v1';
...
let lineIdCounter = 0;
let timerEnabled = false;          // тумблер из настроек: показывать секундомер у задач
const runningTimerIntervals = {};  // taskId -> intervalId, для обновления тикающих кнопок
let categoryDeleteMode = false;          // временный режим удаления категорий (крестики на вкладках)
let categoryDeleteModeTimeout = null;    // таймаут автоотключения режима удаления (10 сек)
...
const enableCategoryDeleteBtn = document.getElementById('enable-category-delete-btn');
...
function logCompletion(type, durationMs = null) {
  const entry = { type, ts: Date.now(), date: toISODate(new Date()) };
...
async function loadAllData() {
  ...
  try {
    ...
    timerEnabled = raw !== null ? raw === '1' : !!(await idbGet(TIMER_ENABLED_KEY));
  } catch (e) {
    timerEnabled = false;
  }
...
// ---------- форматирование длительности (для таймера задач) ----------

// компактный вид mm:ss — используется в тикающей кнопке таймера
...
// человекочитаемый вид (часы/минуты) — используется в статистике
...
function renderTypeTabs() {
  ...
  categories.forEach(cat => {
    ...
    btn.type = 'button';
    btn.className = 'type-tab neumorphic-btn small'
      + (cat === activeType ? ' active' : '')
      + (categoryDeleteMode ? ' deletable' : '');
    btn.dataset.type = cat;

    const label = document.createElement('span');
    label.className = 'type-tab-label';
    label.textContent = cat;
    btn.appendChild(label);

    // крестик удаления показывается только в течение временного режима
    // удаления (см. activateCategoryDeleteMode) — обычно на вкладках его нет
    if (categoryDeleteMode) {
      const del = document.createElement('span');
      del.className = 'type-tab-delete';
      del.textContent = '×';
      del.dataset.type = cat;
      del.setAttribute('role', 'button');
      del.setAttribute('aria-label', 'Удалить категорию ' + cat);
      btn.appendChild(del);
    }
...
// включает режим удаления категорий на 10 секунд: на вкладках появляются
// крестики, по истечении времени они пропадают сами
function activateCategoryDeleteMode() {
  categoryDeleteMode = true;
  if (categoryDeleteModeTimeout) {
    clearTimeout(categoryDeleteModeTimeout);
  }
  renderTypeTabs();

  categoryDeleteModeTimeout = setTimeout(() => {
    categoryDeleteMode = false;
    categoryDeleteModeTimeout = null;
    renderTypeTabs();
  }, 10000);
}
...
function deleteCategory(type) {
  ...
  const message = count > 0
    ? `Удалить категорию «${type}»? Вместе с ней будет удалено ${count} ${pluralTasks(count)}.`
    : `Удалить категорию «${type}»?`;

  if (!confirm(message)) return;

  categories = categories.filter(c => c !== type);
  widgets = widgets.filter(w => w.type !== type);
  saveCategories();
  saveWidgets();

  if (activeType === type) {
    activeType = categories.length > 0 ? categories[0] : null;
  }
...
typeTabsEl.addEventListener('click', (e) => {
  const delEl = e.target.closest('.type-tab-delete');
  if (delEl) {
    e.stopPropagation();
    deleteCategory(delEl.dataset.type);
    return;
  }
...
function renderListBody(el, widget) {
  ...
  widget.tasks.forEach(task => {
    ...
    if (timerEnabled && !task.done) {
      row.appendChild(createTimerButton(widget.id, task));
    } else if (task.done && task.timerElapsedMs) {
      const timeBadge = document.createElement('span');
      timeBadge.className = 'task-time-badge';
      timeBadge.textContent = formatDuration(task.timerElapsedMs);
...
// ---------- секундомер задачи ----------
// Таймер нельзя остановить — только выключить, завершив задачу («Завершить»).
// Пока он запущен, кнопка отключена и просто тикает; время в момент
// завершения записывается в задачу и в историю для статистики.

function createTimerButton(widgetId, task) {
  const btn = document.createElement('button');
  btn.type = 'button';
  ...
  if (task.timerStartedAt) {
    ...
    btn.disabled = true;
    updateTimerButtonLabel(btn, task.timerStartedAt);
    startTimerTicking(widgetId, task.id, btn);
  } else {
    btn.textContent = '⏱ Старт';
    btn.addEventListener('click', () => startTaskTimer(widgetId, task.id));
  }

  return btn;
}

function updateTimerButtonLabel(btn, startedAt) {
  btn.textContent = formatDuration(Date.now() - startedAt);
}

function startTimerTicking(widgetId, taskId, btn) {
  if (runningTimerIntervals[taskId]) {
    clearInterval(runningTimerIntervals[taskId]);
  }

  const widget = widgets.find(w => w.id === widgetId);
  const task = widget && widget.tasks.find(t => t.id === taskId);
  if (!task || !task.timerStartedAt) return;

  runningTimerIntervals[taskId] = setInterval(() => {
    // если строка была убрана из DOM (перерисовка списка, завершение задачи
    // и т.п.) — останавливаем интервал, чтобы не тикать вхолостую
    if (!document.body.contains(btn)) {
      clearInterval(runningTimerIntervals[taskId]);
      delete runningTimerIntervals[taskId];
      return;
    }
    updateTimerButtonLabel(btn, task.timerStartedAt);
  }, 1000);
...
function startTaskTimer(widgetId, taskId) {
  const widget = widgets.find(w => w.id === widgetId);
  if (!widget) return;

  const task = widget.tasks.find(t => t.id === taskId);
  if (!task || task.done || task.timerStartedAt) return;
...
function completeTask(widgetId, taskId) {
  ...
  // выключаем таймер (если был запущен) и фиксируем, сколько ушло на задачу
  let durationMs = null;
  if (task.timerStartedAt) {
    durationMs = Date.now() - task.timerStartedAt;
    task.timerElapsedMs = durationMs;
  ...
  task.done = true;
  saveWidgets();
  logCompletion(widget.type, durationMs);
...
function removeWidget(widgetId) {
  ...
  // виджет уходит без завершения — останавливаем его тикающие таймеры,
  // чтобы не оставлять висящие интервалы
  const widget = widgets.find(w => w.id === widgetId);
  if (widget && Array.isArray(widget.tasks)) {
    widget.tasks.forEach(t => {
      if (runningTimerIntervals[t.id]) {
        clearInterval(runningTimerIntervals[t.id]);
        delete runningTimerIntervals[t.id];
      }
    });
  }
...
function computeStats() {
  ...
  history.forEach(h => {
    ...
    if (h.durationMs) {
      byCategoryTime[h.type] = (byCategoryTime[h.type] || 0) + h.durationMs;
      totalTimeMs += h.durationMs;
    }
  });

  return { total, todayCount, weekTotal, days, weekCounts, streak, byCategory, byCategoryTime, totalTimeMs, todayISO };
}
...
function renderStats() {
  ...
  const cards = [
    ...
    { value: s.totalTimeMs > 0 ? formatDurationHuman(s.totalTimeMs) : '—', label: 'Время с таймером', wide: true }
  ];

  cards.forEach(c => {
    const card = document.createElement('div');
    card.className = 'stat-card' + (c.wide ? ' stat-card-wide' : '');
  ...
  if (catEntries.length === 0) {
    ...
    catEntries.forEach(([type, count]) => {
      ...
      if (s.byCategoryTime[type]) {
        const time = document.createElement('span');
        time.className = 'stats-category-time';
        time.textContent = formatDurationHuman(s.byCategoryTime[type]);
...
enableCategoryDeleteBtn.addEventListener('click', () => {
  // закрываем настройки, чтобы сразу было видно вкладки с крестиками
  closeModal(settingsModal);
  activateCategoryDeleteMode();
});
...
// ---------- тумблер таймера задач ----------

// только обновляет вид переключателя и переменную состояния, без сохранения
// и без перерисовки — используется при первой загрузке
function setTimerToggleUI(enabled) {
  timerEnabled = enabled;
  timerToggle.querySelectorAll('.timer-toggle-btn').forEach(b => {
    b.classList.toggle('active', (b.dataset.timer === 'on') === enabled);
  });
}

// применяет изменение пользователем: сохраняет и обновляет список задач,
// чтобы кнопки секундомера сразу появились/пропали
function applyTimerSetting(enabled) {
  setTimerToggleUI(enabled);
  try {
    localStorage.setItem(TIMER_ENABLED_KEY, enabled ? '1' : '0');
  } catch (e) {
    console.error('Не удалось сохранить настройку таймера:', e);
  }
  idbSet(TIMER_ENABLED_KEY, enabled);
  render();
}

timerToggle.addEventListener('click', (e) => {
  const btn = e.target.closest('.timer-toggle-btn');
  if (!btn) return;
  applyTimerSetting(btn.dataset.timer === 'on');
});
...
// измеряет реальный env(safe-area-inset-top) в пикселях (для отступа хедера
// этого не нужно — там работает чистый CSS max(), но здесь нужно вычесть
// эту величину из высоты капсулы, чтобы не задваивать отступ)
...
function updateOfflineIndicator() {
  ...
  if (!navigator.onLine) {
    // высота капсулы зависит от safe-area на конкретном устройстве
    // (вырез / Face ID / динамический остров); хедер уже отодвинут на
    // саму безопасную зону своим padding-top, поэтому здесь добавляем
    // только "лишнюю" часть капсулы, которая выступает за её пределы —
    // иначе контент отъехал бы вниз вдвое больше, чем нужно
    const indicatorHeight = indicator.offsetHeight;
    const safeTop = getSafeAreaInsetTop();
    const extra = Math.max(0, indicatorHeight - safeTop);
    document.documentElement.style.setProperty('--offline-indicator-extra-height', extra + 'px');

    indicator.classList.add('show');
    scrollable.classList.add('offline-mode');
  } else {
    indicator.classList.remove('show');
    scrollable.classList.remove('offline-mode');
  }
}
